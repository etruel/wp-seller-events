<?php
/**
 * Description of WPeUsuarios: CLASE PARA FUNCIONES DE USUARIOS
 * @author esteban
 */
...
$usuarios = new WPeUsuarios();

if ( class_exists( 'WPeUsuarios' ) ) return;
class WPeUsuarios {
...
//		add_action('user_register',	array(__CLASS__, 'save_user_custom_fields'));
	...
	public static function views_plugin_users($views){
		...
		if ( !is_admin() || (!$pagenow=='user-new.php' && !$pagenow=='user-edit.php') ) 
			return $views;
		$wpse_role = current_user_can('wpse_manager') || current_user_can('wpse_seller') || current_user_can('wpse_customer');
		if($wpse_role){
			$wpse_roles = array('wpse_manager','wpse_seller','wpse_customer');
			$views= array_intersect_key( $views, array_flip($wpse_roles) );
			$args = array(
				'meta_query' => array(
					'key'     => 'wp_capabilities',
					'value'   => $wpse_roles,
			...
			$wpse_users = new WP_User_Query($args);
			$total_users = $wpse_users->get_total();
	...
	public static function just_plugin_roles($editable_roles){
		...
		$wpse_role=current_user_can('wpse_manager') || current_user_can('wpse_seller');
		if($wpse_role){
			$wpse_roles = array('wpse_manager','wpse_seller');
			$editable_roles= array_intersect_key( $editable_roles, array_flip($wpse_roles) );
		}
		return $editable_roles;
	}
			...
			if($wpse_role){
				...
				if( $query->get('role')=="" ) {
					...
					$query->query_where .= " AND ( {$wpdb->usermeta}.meta_key = 'wp_capabilities' AND " . $rolefilter . " ) "; 
						//"CAST( {$wpdb->usermeta}.meta_value AS CHAR) LIKE '%\"wpse\\_seller\"%' ";
...
/*	public static function user_custom_js() {
	...
	public static function modify_contact_methods($profile_fields) {
		$new_fields['address'] = __("Address", 'wpsellerevents' );
		$new_fields['phone'] = __("Telephone", 'wpsellerevents' );
		$new_fields['cellular'] = __("Cellular", 'wpsellerevents' );
		$new_fields['facebook'] = __("Facebook URL", 'wpsellerevents' );
